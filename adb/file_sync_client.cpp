                            ...
                            const char *lpath, const char *rpath)
{
	...
	// d = opendir(lpath);
	//	if(d == 0)
	if (_wchdir(MultiByteToWide(lpath).c_str()))
	{
		fprintf(stderr, "cannot open '%s': %s\n", lpath, strerror(errno));
		return -1;
	}
	...
	do
	{
		wchar_t stat_path[MAX_PATH] = { 0 };
		wchar_t *name = c_file.name;
		...
		*/
		if (strlen(lpath) + lstrlen(c_file.name) + 1 > MAX_PATH)
		{
			_wfindnext(hFile, &c_file);
			continue;
		}
		lstrcpy(stat_path, MultiByteToWide(lpath).c_str());
		lstrcpy(stat_path, c_file.name);
		_wstat(stat_path, &st);

		if (S_IFDIR&st.st_mode) {
			ci = mkcopyinfo(lpath, rpath, WideToMultiByte(name).c_str(), 1);
			ci->next = dirlist;
			dirlist = ci;
		}
		else {
			ci = mkcopyinfo(lpath, rpath, WideToMultiByte(name).c_str(), 0);
			if (_wstat(MultiByteToWide(ci->src).c_str(), &st)) {
				//   closedir(d);
				_findclose(hFile);
